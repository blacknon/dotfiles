...
typeset -g __clock_prompt=""
typeset -g __git_prompt=""

__update_clock_prompt() {
    __clock_prompt="%F{white}[%f%F{yellow}%D{%Y/%m/%d %H:%M:%S (%a) %Z}%f%F{white}]%f"
}
...
precmd_prompt() {
    ...
    if [ ${#__git_prompt} -eq 0 ];then
        TOP_RPROMPT="%F{white}[%fBATTERY:%F{cyan}$BATTERY%f%F{white}]%f "
    else
        TOP_RPROMPT="%F{white}[%fBATTERY:%F{cyan}$BATTERY%f%F{white}]%f ${__git_prompt}"
    fi
...
precmd_functions=""
precmd_functions+=(__update_git_prompt)
precmd_functions+=(precmd_prompt)
typeset -U chpwd_functions
chpwd_functions+=(__update_git_prompt)
__update_clock_prompt
__update_git_prompt
